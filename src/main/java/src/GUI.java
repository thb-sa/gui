...
public class GUI extends javax.swing.JFrame implements
	...
	DefaultListModel<Station> list_station = new DefaultListModel<Station>();
	ZweiwegeClientkommunikator<Aenderungsmeldung, StationAenderung> zCK;
	...
	public GUI() {
		...
		try {
			zCK = new ZweiwegeClientkommunikator<Aenderungsmeldung, StationAenderung>(
					"54.89.87.213",
					new NeuesObjektListener<Aenderungsmeldung>() {
						/*
						...
						public void neuesAustauschobjekt(
							...
							Tageswerte.Darstellung variance = austauschobjekt
									.getTageswerte().getDarstellung();
							...
							a.getAktuelleWerte().put(
									austauschobjekt.getDatum(),
									austauschobjekt.getTageswerte());
							Date_list();
						}
	...
	private void initComponents() {
		...
		javax.swing.GroupLayout panel_datelistLayout = new javax.swing.GroupLayout(
				panel_datelist);
		panel_datelist.setLayout(panel_datelistLayout);
		panel_datelistLayout
				.setHorizontalGroup(panel_datelistLayout
						.createParallelGroup(
						...
						.addGroup(
								panel_datelistLayout
										...
										.addContainerGap()));
		panel_datelistLayout
				.setVerticalGroup(panel_datelistLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(
								panel_datelistLayout
		...
		javax.swing.GroupLayout contenuLayout = new javax.swing.GroupLayout(
				contenu);
		contenu.setLayout(contenuLayout);
		contenuLayout
				.setHorizontalGroup(contenuLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(
								contenuLayout
						...
						.addGroup(
								contenuLayout
										...
										.addContainerGap(
												...
												Short.MAX_VALUE)));
		contenuLayout
				.setVerticalGroup(contenuLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(
		...
		layout.setHorizontalGroup(layout.createParallelGroup(
				...
				contenu, javax.swing.GroupLayout.DEFAULT_SIZE,
				javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));
		layout.setVerticalGroup(layout.createParallelGroup(
				javax.swing.GroupLayout.Alignment.LEADING).addComponent(
				contenu, javax.swing.GroupLayout.Alignment.TRAILING,
				javax.swing.GroupLayout.DEFAULT_SIZE,
				javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));
	...
	private void DiagrammeMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_DiagrammeMouseClicked
		try {
			...
			for (int i = 0; i < l.size(); i++) {
				...
				dateset.setValue(station.getAktuelleWerte().get(date)
						.getRelativeAbweichung(), "", date);
			}

			JFreeChart chart = ChartFactory.createLineChart(
					station_list.getSelectedValue(), "Date", "Variance",
					dateset, PlotOrientation.VERTICAL, false, true, true);
		...
		try {
			if (Actuel.getText().length() != 0) {
				if (date.getText().length() == 10) {
					...
					String dat = d.substring(0, 2) + d.substring(3, 5)
							+ d.substring(6, 10);
					if (test_date(dat)) {
						...
						/*
						 * Hier wird jetzt beispielsweise eine StationsAenderung
						 * versendet.
						 */
						zCK.versende(new StationAenderung(Station_ID.getText(),
								date.getText(), Integer.parseInt(Actuel
										.getText())));

					} else
						JOptionPane.showMessageDialog(null,
								"DateFormat ist Falsch ");
				} else
					JOptionPane.showMessageDialog(null,
							"Date (nach dem Muster TT.MM.JJJJ ausfüllen ");
			} else
				JOptionPane.showMessageDialog(null,
						"Sie muessen Actuel abgeben ");
	...
	private void Date_list() {
		try {
			ArrayList<Date> l = new ArrayList<>();
			DefaultListModel<String> listdate = new DefaultListModel<String>();
			Station station;
	...
	private void Date_listMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_Date_listMouseClicked
		try {
			...
			String wert = Integer.toString(station.getAktuelleWerte()
					.get(Date_list.getSelectedValue()).getAktuellerWert());
			Actuel.setText(wert);
			String var = Integer.toString(station.getAktuelleWerte()
					.get(Date_list.getSelectedValue()).getRelativeAbweichung());
			Tageswerte.Darstellung variance = station.getAktuelleWerte()
					.get(date.getText()).getDarstellung();
			if (variance.equals(Tageswerte.Darstellung.HOCH))
				Variance.setDisabledTextColor(Color.GREEN);
			if (variance.equals(Tageswerte.Darstellung.NIEDRIG))
				Variance.setDisabledTextColor(Color.RED);
	...
	public void neuesAustauschobjekt(Station a) {
		...
		/*
		 * Ueberpruefung, ob mindestens 30 s vergangen sind, seit dem die letzte
		 * Station uebermittelt wurde. Wenn es so ist, dann wird diese Station
		 * nicht mehr als eine von den initial uebergebenen Stationen angesehen,
		 * sondern als neu erzeugte . ==> es muss eine Mitteilung auf der GUI
		 * erzeugt werden, dass es eine neue Station gibt.
		 */
		if (secondValue - firstValue > 30000) {
			new Thread() {
				public void run() {

					newStation.setText("new Station: " + an.getStationID());
					try {
						sleep(3000);
					} catch (Exception e) {
